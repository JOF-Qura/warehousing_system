...
@router.get('/from_procurement', response_model=List[requestSchema.ShowRequest])
def count_response_from_hospital(db: Session = Depends(get_db)):
    query = db.query(requestModel.Request).filter(requestModel.Request.requestor == "Procurement"
                                                    , requestModel.Request.request_status == "Delivered"
                                                    , requestModel.Request.request_type == "To Request").all()
    if not query:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"No Delivered Request Notif from Procurement")
    return query
